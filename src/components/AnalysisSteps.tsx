    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {steps.map((step, index) => {
          const isInProgress = !step.completed && steps[index - 1]?.completed !== false;
          
          return (
            <div 
              key={index}
              className={`flex items-start gap-4 p-4 rounded-lg border relative ${
                isInProgress 
                  ? 'before:absolute before:inset-0 before:rounded-lg before:border-2 before:border-primary/60 before:animate-[border-walk_3s_linear_infinite]' 
                  : ''
              } ${
